...
from data_utils import create_datasets, create_datasets_eng, create_datasets_fra, create_datasets_germ, create_datasets_spa, generate_fold_to_cv_image_dataloaders
import pandas as pd
...
def train(args,model,trainloader,criterion,optimizer):
    ...
    for idx, (data, labels) in enumerate(trainloader):
        ...
        temp_data = data.unsqueeze(1)
        new_data = temp_data.to(args.device, dtype=torch.float32)
...
def main(args):
    ...
    cv_dataloader_generator = generate_fold_to_cv_image_dataloaders(data_type=args.dataset,
